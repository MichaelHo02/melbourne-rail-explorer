...
The centre desk occupies 174 pixels at the bottom of the playfield; the angled side consoles rise to 202 pixels. The left console adds six annunciators driven by accepted simulation state, a working horn button and a sound selector. The right console uses a pale radio-style bezel, LCD service information and blue route/guide/camera keys. It reports the next stop, distance and door/emergency state; it is not a working railway radio or an imitation of unverified safety equipment.

There is one **1280 × 720** composition. The whole canvas and interface scale proportionally together to fit other window dimensions, with letterboxing as required. There is no compact/mobile reflow or alternate arrangement. The previous cab/exterior-view, timetable date, train formation and clock footer remains removed. Timetable provenance remains in the Guide.

The header names its actions: City Loop Route map, Cab, Outside, Centre, Sound, Guide and Pause. The station progress line also opens the route map. Dragging the scene turns the seated cab view or orbits the exterior view; Centre or a double-click resets it. Pointer motion is relative to the scaled canvas, and map/help/pause surfaces gate scene dragging. Cab yaw is limited to 105 degrees either side. Surface exterior views allow a full orbit; the underground exterior view uses a smaller front-quarter envelope inside the authored bore. This is an authored clearance envelope, not collision detection against every station prop.
