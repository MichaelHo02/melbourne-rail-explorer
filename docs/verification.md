...
## Flinders canopy/paving and normal-service observation — 20 September 2026

The production door-check view was compared directly with the saved before image and Marcus Wong's 2019 platform photograph. Broad pale paving slabs are clear; the canopy underside is pale and fine braces follow the existing bays. The generated corrugation relief is assigned to the playable canopy, but does not read clearly from this fixed view. No obvious new clipping or texture seam is visible, and the boarding edge remains readable. This accepts only the bounded paving/canopy-material/frame slice; greater canopy depth and legible corrugation remain visual backlog. A preview sample reported WebGPU, 1,102 draw calls, 2,011,676 triangles, 395 geometries, 257 textures and a 19.26 ms short recent browser-frame interval, with no console warnings or errors. These are local observations, not a controlled performance benchmark.

One normal player service completed Flinders Street–Southern Cross–Flagstaff–Melbourne Central–Parliament, with all four intermediate boarding cycles observed. The return leg reached 552 m remaining at 34 km/h and was paused; this does not establish full-circuit completion. An initial P2 return climb stalled, then the train resumed after changing to P4 and continued climbing. A numeric read-only grade check estimated about 4.69%; P4 climbed it, and no physics defect or code fix was established. The browser session ended and the attempted restore did not complete, so the service proof remains partial. The saved service on port 5180 was left untouched. Detailed local observations are retained in ignored `artifacts/full-service-playtest/` files.
