...
export class HUD {
  ...
  constructor(root:HTMLElement,private actions:Actions,hasSave:boolean){
    ...
    </div><div class="welcome-foot"><span class="key">W</span><span class="key">S</span> Power & brake <span class="control-divider"></span><span class="key">D</span> Doors</div></section>
    ...
    <main id="driving-hud" hidden>
      <span id="camera-hint" class="camera-hint">Drag platform view to look · Double-click to recenter</span><span id="door-check-label" class="door-check-label" role="status" hidden>PLATFORM CAMERA · DOOR CHECK</span>
  ...
  update(sim:Simulation,view:View|'inspection'){
    ...
    this.byId('camera-hint').hidden=view!=='door-check';this.byId('door-check-label').hidden=view!=='door-check';
